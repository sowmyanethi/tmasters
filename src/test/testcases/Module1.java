...
public class Module1 extends UserLibrary
{
	...
	public static void TC12_Myinfo_Addattachment()
	{
		boolean status;
	   //Step 1: login to HRM as a team member
		status = Login(URL, UserName, Password);
		if(status)	
			LogEvent("pass","Member login successfull");		
		else	
			LogEvent("fail","Member login unsuccessfull");
	  //Step 2: Click on My info menu
		status = NavigateMyinfo();
		if(status)	
			LogEvent("pass","Navigated to Myinfo page");		
		else	
			LogEvent("fail","Myinfo page not displayed");
		
	 //Step 3: Click on Add button
		status = ClickElement("//*[@id='btnAddAttachment']");
		if(status)	
			LogEvent("pass","Add button clicked");		
		else	
			LogEvent("fail","Add button not clicked");
	...
	public static void TC_LoginValidation()
	{
		//Write automated steps here for each manual step
		boolean stepstatus;
		//Step 1: Launch HRM application
		stepstatus = OpenUrl(URL);
		if(stepstatus)
			LogEvent("pass", "Application launched");
		else
			LogEvent("fail", "Application not launched");		
		
		//Step 2:
		stepstatus = SetText("//*[@id='txtUsername']",UserName);
		if(stepstatus)
			LogEvent("info", "Enter user name");
		else
			LogEvent("fail", "User name not entered");
		
		stepstatus = SetText("//*[@id='txtPassword']", InvalidPassword);
		if(stepstatus)
			LogEvent("info", "Enter password");
		else
			LogEvent("fail", "password not entered");
		
		stepstatus = ClickElement("//*[@id='btnLogin']");
		if(stepstatus)
			LogEventWithScreeshot("pass", "click Login button");
		else
			LogEventWithScreeshot("fail", "Login button not clicked");
		
		stepstatus = Exist("//span[contains(text='Invalid credentials']");
		if(stepstatus)
			LogEventWithScreeshot("pass", "Invalid Credentials, message displayed");
		else
			LogEventWithScreeshot("fail", "Expected message 'Invalid Credentials' not displayed");
		...
		stepstatus = SetText("//*[@id='txtPassword']", Password);
		if(stepstatus)
			LogEvent("info", "Enter password");
		else
			LogEvent("fail", "password not entered");
		...
		stepstatus = Exist("//span[contains(text='Dashboard']");
		if(stepstatus)
			LogEventWithScreeshot("pass", "HRM Home page displayed");
		else
			LogEventWithScreeshot("fail", "HRM Home page not displayed");
	...
	public static void TC02_AddEmergencyContactsAndVerify()
	{
		...
		if (status)
			LogEventWithScreeshot("pass", "Able to Enter Name successfully");
		else
	...
	public static void TC10_Leave()
	{
		boolean status;
		Login(URL,UserName, Password);
		
		status = fgmousehover(".//*[@id='menu_leave_viewLeaveModule']");
		
		if (status)
			LogEventWithScreeshot("pass", "Able to mouse hover on Leave");
		else
			LogEventWithScreeshot("fail", "UnAble to mouse hover on Leave");
	...
	public static void TC13_LoginHRM()
	{
		Boolean status;
		status = OpenUrl(URL);
		if (status)
			...
			LogEventWithScreeshot("fail", "Unable to Launch facebook Application");
		
		status = SetText("//input[@id='email']", UserName);
		if (status)
			LogEventWithScreeshot("info", "Entered UserName");
		else
			LogEventWithScreeshot("fail", "Unable to Enter UserName");
		
		status = SetText("//input[@id='pass']", Password);
		if (status)
			LogEventWithScreeshot("info", "Entered Password");
		else
			LogEventWithScreeshot("fail", "Unable to Enter Password");

		status = ClickElement("//input[@type='submit']");
		if (status)
			LogEventWithScreeshot("info", "Clicked on Login");
		else
			LogEvent("fail", "Unable to Click on login");
